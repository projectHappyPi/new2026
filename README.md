...
| ffmpeg 트랜스코딩 · 동영상 썸네일 | 원본 그대로 저장, `<video>` 태그로 재생 (서버 트랜스코딩 없음) |
...
- 사진/동영상 업로드 (JPEG/PNG/HEIC/WebP/MP4/MOV), 이미지 200·800·1600px WebP 썸네일 자동 생성
- 날짜별 타임라인, 미디어 뷰어(다운로드/삭제/커버 지정), 소프트 삭제 → 휴지통 → 30일 후 자동 영구 삭제
...
- 동영상은 서버 트랜스코딩/썸네일 없이 원본 그대로 재생됩니다 (ffmpeg 미사용).
